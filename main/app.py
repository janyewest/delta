...
def generate_suggestions_from_sow(sow_id: str):
    ...
    messages: list[ChatCompletionSystemMessageParam | ChatCompletionUserMessageParam] = [
        ChatCompletionSystemMessageParam(role='system', content='You are an expert consulting strategist.'),
        ChatCompletionUserMessageParam(role='user', content=final_prompt),
        ]
...
def generate_suggestions(request: PromptRequest = Body(...)):
    ...
    conn = psycopg2.connect(db_url)
    cur = conn.cursor()
    cur.execute("""SELECT sow_id, embedding
                   FROM consulting.embeddings""")
    rows = cur.fetchall()
    cur.close()
    conn.close()
...
def generate_report(request: PromptRequest):
    ...
    return { 'prompt': prompt, 'report': chat_response.choices[0].message.content.strip() }
...
def save_suggestion(request: dict = Body(...)):
    ...
    response = (
        ...
        .table('saved_suggestions')
        .insert({ 'prompt': prompt, 'suggestions': suggestions })  # List will be serialized as text[]
        .execute()
    )
